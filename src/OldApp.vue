...
const guser = useCurrentUser();

console.log("useor", guser.providerData, auth);

watch(guser, (user, oldVal) => {
  console.log("watch", user.providerData, oldVal);

  setDoc(doc(db, "users", user?.uid), { capital: true }, { merge: true });
});
